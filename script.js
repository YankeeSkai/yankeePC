(function() {
  ...
  function handleDragStart(clientX) {
    ...
    // Игнорируем, если цель — интерактивный элемент
    if (event.target.closest('button, a, input, .hero-content__btn, .catalog-content__btn')) return;
    startX = clientX;
    isDragging = true;
    sliderTrack.style.transition = 'none';
  ...
  const stepDots = document.querySelectorAll('.wizard__step-dot');
  let currentStep = 1;
  let wizardData = {};
  ...
  function showWizardSlide(step) {
    ...
    const activeDot = document.querySelector(`.wizard__step-dot[data-step="${step}"]`);
  ...
  function showContactField(type) {
    resetContactFields();
    if (contactFields[type]) contactFields[type].style.display = 'block';
  }
      ...
      .then(() => {
        ...
        body.innerHTML = `<div style="text-align:center; padding:2rem;">
          <svg width="80" height="80" viewBox="0 0 80 80">
            <circle cx="40" cy="40" r="36" fill="none" stroke="#ff3333" stroke-width="4" stroke-dasharray="226" stroke-dashoffset="226" style="animation: draw-circle 0.6s ease forwards"/>
            <polyline points="25,40 36,52 55,28" fill="none" stroke="#ff3333" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" stroke-dasharray="50" stroke-dashoffset="50" style="animation: draw-check 0.4s 0.5s ease forwards"/>
          </svg>
          <p style="color:#ccc; margin-top:1rem;">Заявка отправлена!<br>Мы свяжемся с вами.</p>
        </div>`;
